    ...
    scale(scale){
        if(this._rigidBody){
            ...
            if(collider2D.NAME() === "CircleCollider2D"){
                ...
                console.log("WWW",collider2D.center())
                collider2D.scale(scale.x, scale.y, collider2D.center());
            }
        }
        
        this._transform.setScale(new Vector2D(this._transform.scale().x + scale.x, this._transform.scale().y + scale.y) );
...
class SysCollision2D{
    ...
    detecteCollision(composite, updateStateOfCollisionChild = true){
        ...
        if(composite.collisionBetweenChildrens()){
            composite.childrens().forEach( (indice, child) => {
                ...
                for(let j = indice + 1; j < composite.NumberOfChildren(); i++){
...
function testSprite(R){
    ...
    C1.drawPost = (ctx)=>{
        Dibujo.color = "red";
        ctx.restore()
        Dibujo.drawRectangulo(ctx, new Vector2D(C1.AABB().min_x, C1.AABB().min_y), new Vector2D(C1.AABB().max_x - C1.AABB().min_x, C1.AABB().max_y - C1.AABB().min_y));
    }

    C2.drawPre = (ctx)=>{
        Dibujo.color = "red";
        Dibujo.drawRectangulo(ctx, new Vector2D(C2.AABB().min_x, C2.AABB().min_y), new Vector2D(C2.AABB().max_x - C2.AABB().min_x, C2.AABB().max_y - C2.AABB().min_y));
    }

    C1.rotate(50);
    C1.traslate(new Vector2D(50,20));
    console.log(C1.AABB());
    C1.scale(new Vector2D(20,20));
    console.log(C1.AABB());
    ecena.add(C1);
    ecena.add(C2);
    ...
    setTimeout(()=>{
        C1.traslate(new Vector2D(25,25));
        CanvasRender.render(ecena); 
        C1.traslate(new Vector2D(25,25));
        CanvasRender.render(ecena);

        C1.scale(new Vector2D(25,25));
        CanvasRender.render(ecena); 
        C1.scale(new Vector2D(25,25));
        CanvasRender.render(ecena);
