...
class Vertices2D{
    ...
    setVertices(arrayPoints){
        ...
        if(arrayPoints && arrayPoints.length > 2){
            ...
            this.#points = []
            arrayPoints.forEach(point => this.#points.push(new Vector2D(point.x, point.y)))
...
class PoligonCollider2D extends Collider2D{
    ...
    constructor(centre = new Vector2D(0,0), vertices){
        ...
        if(vertices && vertices.NAME && vertices.NAME() == "Vertices2D"){
            ...
            this.#bound = this.AABB(vertices.pointsCopy());
    ...
    verticesRef(){
        return this.#vertices.pointsRef();
    }

    verticesCopy(){
        return this.#vertices.pointsCopy(); 
    }
...
class RectangleCollider2D extends PoligonCollider2D{
    ...
    //centre : Vector2D
    constructor(centre = new Vector2D(0,0) , width = 4, height = 4){
...
class AABBCollider2D extends RectangleCollider2D{
    ...
    constructor(center = new Vector2D(0,0), width = 4, height = 4){
...
class CollisionFilter {
    ...
    // Camara group -5 y category = 1 reservado para la camara y todo los componentes por defecto tendran la masck = 1 para colisionar con la camara y determinar si se encuentra dentro
    ...
    constructor(group = 0, category = 2, mask = [1, 2], type = CollisionFilter.TYPES().COMPONENT){
        ...
        if(Utilities.controlUseNumberType([group, category, mask])){
...
class Tranform {
    ...
    #position; // Posicion en la scena
    #scale; // escala a aumentar de sprite y rigidbody
    #angle; // angulo a tener sprite y rigidbody
    ...
    setDestinoMove(vectorDestino){
        if(Utilities.controlUseVector2DType(vectorDestino)){
            this.#destinyMoveRef = vectorDestino;
        }
    }
...
class Resources {
    ...
    audio(keyName){
        ...
        this.#audios.forEach(A => {
            if(A.name ===  keyName)
                ...
                result = {"audio": A.audio, "complete":complete};
                return;
        }) 
        return result;
    }
    ...
    image(keyName){
        ...
        this.#images.forEach(img => {
            if(img.name ===  keyName)
                result = {"image": img.image, "complete": img.image.complete,}
                return
        }) 
...
class Sprite{
    ...
    #scale; // dimenciones de corte (alto y ancho desde la pocicion offset)
    ...
    addAnimation(animationSprite){
        if(Utilities.controlUseAnimationSpriteType(animationSprite)){
            this.#animations.push(animationSprite);
        }
    }
    ...
    setCurrentAnimation(tagName){
        ...
        this.#offset = newCurrentAnimation.offset();
        this.#scale = newCurrentAnimation.scale();
    ...
    removeAnimation(tagName){
        this.#animations.forEach( (animation, indice) => {
            if(animation.tag() === tagName){
              this.#animations.splice(indice, 1);
              return;
            }
        })
    }
...
class Component{
    ...
    #render;
    #layer = 0; // Define la profundiad de pintado respecto a otros elementos
    ...
    setTransform(transform){
        if(Utilities.controlUseRigidBodyType(transform)){
    ...
    transform(){
        let copyTranform;
        return Utilities.variableCopy(copyTranform, this.#transform);
    }

    rigidBody(){
        let copyRigidBody;
        return Utilities.variableCopy(copyRigidBody, this.#rigidBody);
    }

    sprite(){
        let copySprite;
        return Utilities.variableCopy(copySprite, this.#sprite);
    }
    ...
    scale(scale){
        ...
        let collider2D = this.#rigidBody.collider2D();

        if(collider2D.NAME() === "CircleCollider2D"){
            collider2D.scale(scale.x);    
        }else{
            collider2D.scale(scale.x, scale.y, collider2D.centerCopy());
        }

        this.#transform.setScale(this.#transform.scale().x + scale.x, this.#transform.scale().y + scale.y);
    ...
    rotate(angleRad){
        ...
        this.#transform.setAngle(angleRad); //Angulo que tomara los sprites al ser pintados
    ...
    traslate(desplacemnt){
        ...
        this.#transform.setPosition(this.#transform.position().x + desplacemnt.x, this.#transform.position().y + desplacemnt.y)
    }
    ...
    colisionFilter(){
        return this.#rigidBody.colisionFilter();
    }
...
class Composite {
    ...
    #collisionBetweenChildrens = true;
    ...
    rotate(angleRad){
        this.#childrens.forEach(element => element.rotate(angleRad));
    }

    scale(scale){
        this.#childrens.forEach(element => element.scale(scale));
    }

    traslate(desplacement){
        this.#childrens.forEach(element => element.traslate(desplacement));
    }
    ...
    findElements(tagName){
        return this.#childrens.filter(element => element.tag() === tagName);
    }
    ...
    AABB(){
        ...
        if(this.#bound){
            this.#bound = this.calculateAABB(this.#childrens)    
        }
    ...
    setBounds(topLeft, bottomRight){
        if(Utilities.controlUseVector2DType(topLeft) && Utilities.controlUseVector2DType(bottomRight)){
            this.#bound = {min_x: topLeft.x , min_y: topLeft.y, max_x: bottomRight.x, max_y: bottomRight.y}
        }
    ...
    setCollisionBetweenChildrens(boolean){
        if(Utilities.controlUseBooleanType(boolean)){
            this.#collisionBetweenChildrens = boolean;
        }
    }

    collisionBetweenChildrens(){
        return this.#collisionBetweenChildrens;
    }
