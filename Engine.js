...
class CircleCollider2D extends Collider2D{
    ...
    constructor(centre = new Vector2D(0,0) , radio = 0){
...
class AABBCollider2D extends Collider2D{

    //Este collider no puede ser rotado
    #bounds
    #NAME = "AABBCollider2D"

    constructor(center = new Vector2D(0,0), vector2D1 = new Vector2D(0,0), vector2D2 = new Vector2D(10,10)){
        
        super(center);

        if(Utilities.controlUseVector2DType(vector2D1) && Utilities.controlUseVector2DType(vector2D2)){
            this.#bounds = {
                min_x : vector2D1.x, 
                min_y : vector2D1.y, 
                max_x : vector2D2.x, 
    ...
    traslate(position){
        if(Utilities.controlUseVector2DType(position)){
            this.#bounds.min_x += position.x;
            this.#bounds.max_x += position.x;
            this.#bounds.min_y += position.x;
            this.#bounds.max_y += position.x;
        } 
    }
...
class CollisionFilter {
    ...
    constructor(group = 0, category = 1, mask = [1], type = CollisionFilter.TYPES().COMPONENT){
...
class Sprite{
    ...
    //animations;
    constructor(image, xyScale, xyOffset, nroFrame = 0) {
